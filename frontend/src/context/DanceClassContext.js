...
export const danceclassReducer = (state, action) => { //previous state (reliable state value), action is object passed into dispatch function
    ...
    switch(action.type) {
        ...
        case 'UPDATE_DANCECLASS':
            return {
                danceclasses: state.danceclasses ? state.danceclasses.map((danceclass) => // loop through all exisitng classes
                ...
                ) : [action.payload] // if danceclasses is null (not loaded, return single-item array with the updated class instead of crash)
            }
